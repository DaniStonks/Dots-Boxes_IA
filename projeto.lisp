...
(defun escrever-no (no-solucao)
  ...
  (with-open-file (stream "log.dat"
                         :direction :output
                         :if-exists :append
                         :if-does-not-exist :create)
  (format stream "Estado inicial: ~A | Solução encontrada: ~A | Número de nós gerados: ~A | Número de nós expandidos: ~A" (estado-no-inicial no-solucao)(no-solucao)(+ (length *abertos*)(length *fechados*)) (length *fechados*))))
...
(defun estado-no-inicial(no-solucao)
  ...
  (cond ((null (no-pai no-solucao)) (no-estado no-solucao))
        (t (estado-no-inicial (no-pai solucao))))
)